#!/usr/bin/env python3
"""Query Aster Futures V3 order by orderId/clientOrderId."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Query Aster V3 order")
...
def main() -> int:
    try:
        ...
        code, body = client.signed_request("GET", "/fapi/v3/order", params)
        if args.verbose:
            out = format_exchange_response(code, body)
        else:
            compact = compact_order_fields(body) if isinstance(body, dict) else body
            out = {"status_code": code, "body": compact}
        print(json.dumps({"query_order": out}, ensure_ascii=False))
